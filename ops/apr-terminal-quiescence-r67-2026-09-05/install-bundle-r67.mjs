...
const ops = path.join(repository, "ops/apr-terminal-quiescence-r67-2026-09-05");
...
const versionId = "b52ec569-terminal-cohort-quiescence-r67-20260906";
const versionDirectory = path.join(canonicalRoot, "versions", versionId);
const current = path.join(canonicalRoot, "current");
const expectedPrevious = path.join(canonicalRoot, "versions/e4afbcb6-final-canonical-proof-r66-20260905");
...
const assert = (condition, reason) => { if (!condition) throw new Error(`apr_r67_install_failed:${reason}`); };
...
if (previousTarget !== versionDirectory) {
  const temporary = path.join(canonicalRoot, `.current-r67-${randomUUID()}`);
...
const receipt = {
  version: "apr-terminal-cohort-quiescence-bundle-install-receipt-r67-v1",
...
writeFileSync(path.join(ops, "bundle-install-receipt-r67.json"), `${JSON.stringify(receipt, null, 2)}\n`, "utf8");
