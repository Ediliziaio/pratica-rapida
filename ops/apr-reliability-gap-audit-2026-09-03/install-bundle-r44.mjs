...
const preinstallPath = path.join(ops, "preinstall-gate-attestation-r44.json");
...
const versionId = "11757a99-fresh-proof-before-terminal-r44-20260904";
const versionDirectory = path.join(canonicalRoot, "versions", versionId);
const current = path.join(canonicalRoot, "current");
const expectedPrevious = path.join(canonicalRoot, "versions/1af2d3b0-stale-probe-resume-r43-20260904");
...
const assert = (condition, reason) => { if (!condition) throw new Error(`apr_r44_install_failed:${reason}`); };
...
assert(preinstall.monotonicGate?.candidate?.tests === 1726 && preinstall.monotonicGate?.rulesProved === 118, "preinstall_gate_incomplete");
assert(preinstall.correction?.status === "proved_positive_negative_fresh_checkpoint_before_terminal", "correction_gate_incomplete");
...
if (previousTarget !== versionDirectory) {
  const temporary = path.join(canonicalRoot, `.current-r44-${randomUUID()}`);
...
const receipt = {
  version: "apr-screening-fresh-proof-before-terminal-r44-bundle-install-receipt-v1",
...
const receiptPath = path.join(ops, "bundle-install-receipt-r44.json");
