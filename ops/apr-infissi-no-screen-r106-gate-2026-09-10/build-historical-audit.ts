...
const gateRoot = path.join(repositoryRoot, "ops/apr-infissi-no-screen-r106-gate-2026-09-10");
const baselineBundle = path.join(canonicalRoot, "versions/c0826cfe-multi-invoice-parser-r105-20260910");
const evidenceRoot = path.join(gateRoot, "rule-evidence");
const currentBundle = path.join(gateRoot, "staged-bundle");
const output = path.join(gateRoot, "historical-audit-report-r106.json");
...
if (report.status !== "incomplete" || report.unresolvedDocumentedDecisionCount !== authorized.length || JSON.stringify(unresolved) !== JSON.stringify(authorized)) {
...
process.stdout.write(`${JSON.stringify({
  ...
  newlyActivatedNow: report.decisions.filter((item) => item.recoveryStatus === "newly_activated_now").map((item) => item.decisionId),
  unresolved,
  output,
