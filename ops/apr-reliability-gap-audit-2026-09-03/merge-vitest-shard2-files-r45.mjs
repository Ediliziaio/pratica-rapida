...
const reports = Array.from({ length: 28 }, (_, index) => JSON.parse(readFileSync(
  path.join(directory, `vitest-shard-2-file-${String(index + 1).padStart(2, "0")}-r45.json`),
  "utf8",
)));
...
writeFileSync(path.join(directory, "vitest-shard-2-r45.json"), `${JSON.stringify(merged, null, 2)}\n`);
