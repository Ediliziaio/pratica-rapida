...
const expectedVersionId = "de26daa2-producer-position-r36-20260903";
...
const assert = (condition, reason) => { if (!condition) throw new Error(`apr_r36_postinstall_failed:${reason}`); };

const receipt = readJson(path.join(ops, "bundle-install-receipt-r36.json"));
...
const artifact = {
  version: "apr-producer-position-table-r36-postinstall-attestation-v1",
  ...
  activeBundleSha256,
  activationBoundary: { canonicalPointerSelected: true, existingKeepaliveProcessWasNotRestarted: true, residentKeepaliveCodePredatesR36: true, nextFreshAprProcessWillLoadR36: true, operationalR36ReplayPerformed: false },
...
const target = path.join(ops, "postinstall-attestation-r36.json");
