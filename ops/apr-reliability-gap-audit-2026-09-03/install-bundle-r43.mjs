...
const preinstallPath = path.join(ops, "preinstall-gate-attestation-r43.json");
...
const versionId = "1af2d3b0-stale-probe-resume-r43-20260904";
const versionDirectory = path.join(canonicalRoot, "versions", versionId);
const current = path.join(canonicalRoot, "current");
const expectedPrevious = path.join(canonicalRoot, "versions/cc879457-probe-lifecycle-r42-20260904");
...
const assert = (condition, reason) => { if (!condition) throw new Error(`apr_r43_install_failed:${reason}`); };
...
assert(preinstall.monotonicGate?.candidate?.tests === 1725 && preinstall.monotonicGate?.rulesProved === 117, "preinstall_gate_incomplete");
assert(preinstall.correction?.status === "proved_positive_negative_identity_bound_retirement", "correction_gate_incomplete");
...
if (previousTarget !== versionDirectory) {
  const temporary = path.join(canonicalRoot, `.current-r43-${randomUUID()}`);
...
const receipt = { version: "apr-uncertain-save-stale-result-retirement-r43-bundle-install-receipt-v1", installedAt: new Date().toISOString(), status: "bundle_promoted_local_gate_verified", preinstallGateSha256: sha256(preinstallPath), versionId, previousTarget, currentTarget: versionDirectory, installedBundle: preinstall.stagedBundle, sequencer: preinstall.sequencer, keepaliveRestartRequested: false, chromeRestartRequested: false };
const receiptPath = path.join(ops, "bundle-install-receipt-r43.json");
