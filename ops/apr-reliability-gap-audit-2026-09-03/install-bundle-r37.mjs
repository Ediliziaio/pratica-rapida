...
const preinstallPath = path.join(ops, "preinstall-gate-attestation-r37.json");
...
const versionId = "8c9c921d-single-product-energy-r37-20260903";
...
const assert = (condition, reason) => { if (!condition) throw new Error(`apr_r37_install_failed:${reason}`); };
...
assert(preinstall.monotonicGate?.candidate?.tests >= 1700 && preinstall.monotonicGate?.rulesProved === 112, "preinstall_gate_incomplete");
assert(JSON.stringify(preinstall.freshOriginalReplay?.improvedCases) === JSON.stringify(["gemma-minore"]) && preinstall.freshOriginalReplay?.unchangedCases === 99, "replay_gate_incomplete");
...
if (previousTarget !== versionDirectory) {
  const temporary = path.join(canonicalRoot, `.current-r37-${randomUUID()}`);
...
const receipt = {
  version: "apr-single-product-energy-r37-bundle-install-receipt-v1",
...
const receiptPath = path.join(ops, "bundle-install-receipt-r37.json");
