...
const expectedVersionId = "1af2d3b0-stale-probe-resume-r43-20260904";
...
const assert = (condition, reason) => { if (!condition) throw new Error(`apr_r43_postinstall_failed:${reason}`); };
const receipt = readJson(path.join(ops, "bundle-install-receipt-r43.json"));
...
assert(["setup_ready", "running"].includes(service.status) && service.chromePid > 0 && Date.now() - Date.parse(service.heartbeatAt) < 60_000, "persistent_heartbeat");
...
const artifact = {
  version: "apr-uncertain-save-stale-result-retirement-r43-postinstall-attestation-v1",
  verifiedAt: new Date().toISOString(), status: "installed_selected_session_preserved_operational_replay_pending", versionId: expectedVersionId,
  tripleVerification: {
  ...
  activeBundleSha256,
  activationBoundary: { canonicalPointerSelected: true, existingKeepaliveProcessWasNotRestarted: true, residentKeepaliveCodePredatesR43: true, nextFreshAprProcessWillLoadR43: true, operationalR43ReplayPerformed: false },
...
const target = path.join(ops, "postinstall-attestation-r43.json");
