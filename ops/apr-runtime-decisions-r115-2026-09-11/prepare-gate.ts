...
const gateRoot = path.join(repositoryRoot, "ops/apr-runtime-decisions-r115-2026-09-11");
...
const historicalAudit = path.join(gateRoot, "historical-audit-report-r115.json");
...
const readGreenReport = (target: string) => {
  ...
  if (!report.success || report.numFailedTests !== 0 || report.numPendingTests !== 0 || report.numPassedTests !== report.numTotalTests) throw new Error(`r115_source_test_report_not_green:${path.basename(target)}`);
  return report;
};
...
for (const [executable, ruleIds] of Object.entries(requiredRuntimeRules)) {
  ...
  if (missing.length) throw new Error(`r115_runtime_rules_missing:${executable}:${missing.join(",")}`);
}
...
if (evidence.testedCount !== APR_RULE_TEST_MATRIX.length) throw new Error(`r115_rule_evidence_incomplete:${evidence.testedCount}/${APR_RULE_TEST_MATRIX.length}`);
...
if (audit.status !== "incomplete" || audit.unresolvedDocumentedDecisionCount !== APR_AUTHORIZED_HISTORICAL_GAP_DECISION_IDS.length || !sameStrings(unresolved, APR_AUTHORIZED_HISTORICAL_GAP_DECISION_IDS)) {
  throw new Error(`r115_historical_gap_not_exactly_authorized:${JSON.stringify(unresolved)}`);
}
