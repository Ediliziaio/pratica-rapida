...
const expectedVersionId = "f68568da-fiscal-copy-reliability-r35-20260903";
...
const assert = (condition, reason) => { if (!condition) throw new Error(`apr_r35_postinstall_failed:${reason}`); };

const receipt = readJson(path.join(ops, "bundle-install-receipt-r35.json"));
...
const artifact = {
  version: "apr-fiscal-copy-reliability-r35-postinstall-attestation-v1",
  verifiedAt: new Date().toISOString(),
  status: "installed_selected_session_preserved_operational_replay_pending",
  versionId: expectedVersionId,
  tripleVerification: {
    launchctlAndProcesses: {
      keepaliveLabel: "com.praticarapida.apr-enea-immortal-keepalive",
      state: "running",
      pid: service.processPid,
      chromeAprPid: service.chromePid,
      chromeRestarted: false,
      keepaliveRestarted: false,
    },
    persistentCheckpointAndJournal: {
      serviceVersion: service.version,
      revision: service.revision,
      status: service.status,
      heartbeatAt: service.heartbeatAt,
      reason: service.reason,
      contractObservedAt: driver.contract.observedAt,
      contractEvidenceId: driver.contract.evidenceId,
    ...
    dashboardOrApi: {
      endpoint: "http://127.0.0.1:9331/json/list",
      targetId: page.id,
  ...
  activeBundleSha256,
  activationBoundary: {
    canonicalPointerSelected: true,
    existingKeepaliveProcessWasNotRestarted: true,
    residentKeepaliveCodePredatesR35: true,
    nextFreshAprProcessWillLoadR35: true,
    operationalR35ReplayPerformed: false,
  },
  safety: {
    eneaMutationPerformed: false,
...
const target = path.join(ops, "postinstall-attestation-r35.json");
