-- Correzione della guardia pagamento in submit_form_by_token
-- (20260901000002): deve valere SOLO per il servizio completo.
--
-- Con "documenti forniti + form online" chi compila il modulo e' il
-- RIVENDITORE, al posto dei moduli cartacei: bloccargli l'invio finche' il
-- SUO cliente non paga significa bloccare il lavoro di chi i soldi non li
-- deve. Il pagamento del cliente viaggia in parallelo (email + /paga) e lo
-- staff lo vede dal badge "Non pagata" sulla card.
...
DECLARE
  ...
  v_tipo_servizio text;
  v_tipo_fatturazione text;
  v_pagamento_stato text;
BEGIN
  SELECT id, brand::text, tipo_servizio::text, tipo_fatturazione::text, pagamento_stato::text
    INTO v_practice_id, v_brand, v_tipo_servizio, v_tipo_fatturazione, v_pagamento_stato
  ...
  -- Servizio completo a carico del cliente finale: prima il pagamento,
  -- poi l'invio del modulo. (/paga/:token e /form/:token condividono il
  -- token: senza questo blocco basta cambiare il path nell'URL.)
  IF v_tipo_servizio = 'servizio_completo'
     AND v_tipo_fatturazione = 'cliente_finale'
