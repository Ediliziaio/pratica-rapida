...
const root = path.join(repositoryRoot, "ops/apr-operator-response-ledger-concurrency-r117-2026-09-12");
const reportPath = path.join(root, "full-vitest-report-sequential.json");
const evidenceRoot = path.join(root, "rule-evidence");
const stagedBundle = path.join(root, "staged-bundle");
const historicalAuditPath = path.join(root, "historical-audit-report-r117.json");
...
const concurrencyRuleId = "system-operator-response-ledger-concurrency-v1";
...
if (!report.success || report.numFailedTests !== 0 || report.numPendingTests !== 0 || report.numPassedTests !== report.numTotalTests) {
  throw new Error("r117_full_test_report_not_green");
}
...
for (const executable of ["apr-supervisor.mjs", "apr-enea-worker.mjs"] as const) {
  if (!readFileSync(path.join(stagedBundle, executable), "utf8").includes(concurrencyRuleId)) {
    throw new Error(`r117_runtime_rule_missing:${executable}:${concurrencyRuleId}`);
  }
}
...
const evidence = materializeAprRuleProofs({
  ...
  testCommand: `suite completa sequenziale ${report.numPassedTests}/${report.numTotalTests}`,
});
if (evidence.testedCount !== APR_RULE_TEST_MATRIX.length) {
  throw new Error(`r117_rule_evidence_incomplete:${evidence.testedCount}/${APR_RULE_TEST_MATRIX.length}`);
}
...
if (audit.status !== "incomplete"
  ...
  throw new Error(`r117_historical_gap_not_exactly_authorized:${JSON.stringify(unresolved)}`);
}
...
const summary = {
  status: "PASS",
  tests: { total: report.numTotalTests, passed: report.numPassedTests, failed: 0, pending: 0 },
  matrixRules: APR_RULE_TEST_MATRIX.length,
  testedRules: evidence.testedCount,
  requiredRuntimeRule: concurrencyRuleId,
