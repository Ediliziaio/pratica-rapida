...
export interface WorkDateSource {
  date: string;
  sourceDocumentId: string;
  kind: "explicit_start" | "explicit_completion" | "pertinent_document";
}
...
export function resolveWorkDates(sources: readonly WorkDateSource[]):
  ...
  const documents = valid.filter((item) => item.kind === "pertinent_document")
    .sort((a, b) => Date.parse(a.date) - Date.parse(b.date));
  const start = explicitStart ?? documents[0];
  const end = explicitEnd ?? documents.at(-1);
