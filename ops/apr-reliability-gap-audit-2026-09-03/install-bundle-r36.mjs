...
const preinstallPath = path.join(ops, "preinstall-gate-attestation-r36.json");
...
const versionId = "de26daa2-producer-position-r36-20260903";
const versionDirectory = path.join(canonicalRoot, "versions", versionId);
const current = path.join(canonicalRoot, "current");
const expectedPrevious = path.join(canonicalRoot, "versions/f68568da-fiscal-copy-reliability-r35-20260903");
...
const assert = (condition, reason) => { if (!condition) throw new Error(`apr_r36_install_failed:${reason}`); };
...
assert(preinstall.monotonicGate?.candidate?.tests === 1698 && preinstall.monotonicGate?.rulesProved === 111, "preinstall_gate_incomplete");
assert(JSON.stringify(preinstall.freshOriginalReplay?.improvedCases) === JSON.stringify(["massimo-cappello"]) && preinstall.freshOriginalReplay?.unchangedCases === 99, "replay_gate_incomplete");
...
if (previousTarget !== versionDirectory) {
  const temporary = path.join(canonicalRoot, `.current-r36-${randomUUID()}`);
...
const receipt = {
  version: "apr-producer-position-table-r36-bundle-install-receipt-v1",
...
const receiptPath = path.join(ops, "bundle-install-receipt-r36.json");
