...
describe("coda audit multi-pratica locale", () => {
  ...
  it("riprende ogni pratica dallo stesso checkpoint conservando fonti, regole, blocco e audit", () => {
    const checkpointed = checkpointOperationalPractice(DEFAULT_AUDITED_OPERATOR_QUEUE, "audit-samuele-colombo", {
      ...
      ruleId: "authorized-16-schermature-meccanismo",
      block: null,
    ...
    expect(samuele.appliedRules).toContain("authorized-16-schermature-meccanismo");
    expect(samuele.audit.at(-1)).toMatchObject({ type: "queue_resumed", step: "screenings", appliedRuleIds: ["system-atomic-checkpoint-resume"] });
  });
