...
// Replay locale wide100 POST-ricalcolo completo (r85): a differenza di r71/r73
...
// scritto nei checkpoint dal ricalcolo forzato (full-recalc-r85.mjs): e' il
...
// --- 1. Confermare che il bundle installato e' davvero quello costruito da questo sorgente (r85) ---
if (!existsSync(currentPointer) || !lstatSync(currentPointer).isSymbolicLink()) throw new Error("apr_r85_replay_current_pointer_missing");
const installedVersionId = path.basename(path.resolve(canonicalRoot, readlinkSync(currentPointer)));
const installReceiptPath = path.resolve(import.meta.dirname, "../apr-install-manso-cap-typo-fix-r85-2026-09-07/install-receipt.json");
const installReceipt = JSON.parse(readFileSync(installReceiptPath, "utf8"));
if (installReceipt.versionId !== installedVersionId) throw new Error(`apr_r85_replay_version_mismatch:receipt=${installReceipt.versionId}:disk=${installedVersionId}`);
for (const name of Object.keys(installReceipt.installedBundle)) {
  const activePath = path.join(currentPointer, name);
  if (sha256File(activePath) !== installReceipt.installedBundle[name]) throw new Error(`apr_r85_replay_active_hash_mismatch:${name}`);
}
...
const payload = {
  version: "apr-wide100-local-replay-r85-v1",
...
if (!verifyImmutableArtifactEnvelope(envelope)) throw new Error("apr_r85_replay_envelope_self_check_failed");
...
const reportMd = `# Replay locale wide100 — r85 (post-ricalcolo completo, stato persistito, hash-verificato)
...
- Fonte: stato persistito nei checkpoint dopo il ricalcolo forzato completo (full-recalc-r85.mjs), non un bypass che ricalcola in isolamento.
