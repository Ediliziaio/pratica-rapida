...
export class PersistentRuleMatrixEvidence {
  ...
  constructor(rootDirectory: string) { this.file = path.join(path.resolve(rootDirectory), "rule-test-evidence.json"); }
  load(): AprRuleMatrixEvidence | null {
    ...
    try {
      ...
      return evidence.matrixVersion === APR_RULE_TEST_MATRIX_VERSION && evidence.registryVersion === ENEA_OPERATIONAL_REGISTRY_VERSION
        ...
        && Array.isArray(evidence.passedKeys) && evidence.ruleProofs && typeof evidence.ruleProofs === "object" ? evidence : null;
    } catch { return null; }
  }
  ...
  recordRuleProof(input: {
    ...
    if (!input.positiveTest.trim() || !input.negativeTest.trim() || !input.testCommand.trim()) throw new Error("Test positivo, negativo e comando obbligatori.");
