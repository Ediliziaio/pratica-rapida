...
// Replay locale wide100 POST-ricostruzione completa (r89): legge lo STATO
// REALE gia' scritto nei checkpoint dalla ricostruzione forzata e mai
// selettiva (full-reconstruction-r89.mjs, che applica applyValidationRevision
...
// pratica che la revisione di validazione r89 risulti davvero applicata nel
...
const EXPECTED_VALIDATION_REVISION = "apr-full-reconstruction-r89-2026-09-08";

// --- 1. Confermare che il bundle installato e' davvero quello costruito da questo sorgente (r89) ---
if (!existsSync(currentPointer) || !lstatSync(currentPointer).isSymbolicLink()) throw new Error("apr_r89_replay_current_pointer_missing");
...
if (installReceipt.versionId !== installedVersionId) throw new Error(`apr_r89_replay_version_mismatch:receipt=${installReceipt.versionId}:disk=${installedVersionId}`);
for (const name of Object.keys(installReceipt.installedBundle)) {
  const activePath = path.join(currentPointer, name);
  if (sha256File(activePath) !== installReceipt.installedBundle[name]) throw new Error(`apr_r89_replay_active_hash_mismatch:${name}`);
}
...
// anche una sola pratica non porta la revisione r89 tra quelle applicate, il
// replay si rifiuta di firmare un numero che potrebbe essere basato su una
// cache stantia.
if (notGenuinelyReconstructed.length > 0) throw new Error(`apr_r89_replay_not_genuinely_reconstructed:${JSON.stringify(notGenuinelyReconstructed.map((r) => r.customerKey))}`);
...
const payload = {
  version: "apr-wide100-local-replay-r89-v1",
...
if (!verifyImmutableArtifactEnvelope(envelope)) throw new Error("apr_r89_replay_envelope_self_check_failed");
...
const reportMd = `# Replay locale wide100 — r89 (post-ricostruzione completa mai selettiva, stato persistito, hash-verificato)
...
- Fonte: stato persistito nei checkpoint dopo la ricostruzione forzata e mai selettiva di TUTTE le 100 pratiche (full-reconstruction-r89.mjs); ogni pratica verificata portare la revisione ${EXPECTED_VALIDATION_REVISION} tra quelle applicate, prova che non e' un verdetto vecchio mai ricalcolato.
- Correzioni incluse in questo giro (nessuna delle pratiche del campione wide100 e' toccata direttamente da queste sei, verificato: 0 variazioni rispetto a r88): preposizione "a"/"da" e singolare "Zanzariera" nel saldo LM Tende (Pezzani); misura da modulo d'ordine tecnico VA.ILA. per pergotenda (Pasinato); tre varianti di layout Linea Sole Potito per "Tenda modello ... L WxH" (Di Bello, Madia, Bellotti); intestazione fattura con data punteggiata con priorita' su un riferimento narrativo nel corpo (Cirillo). Verifica di isolamento richiesta esplicitamente da Giuliano: nessuna pratica gia' pronta si e' rotta per una correzione successiva.
