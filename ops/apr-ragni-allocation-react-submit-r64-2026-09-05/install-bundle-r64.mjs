...
const ops = path.join(repository, "ops/apr-ragni-allocation-react-submit-r64-2026-09-05");
...
const versionId = "3c3f94d6-allocation-react-submit-r64-20260905";
const versionDirectory = path.join(canonicalRoot, "versions", versionId);
const current = path.join(canonicalRoot, "current");
const expectedPrevious = path.join(canonicalRoot, "versions/322df1b9-allocation-single-intent-r63-20260905");
const bundleFiles = [
  "apr-supervisor.mjs",
  "apr-enea-worker.mjs",
  "apr-watchdog.mjs",
...
const assert = (condition, reason) => {
  if (!condition) throw new Error(`apr_r64_install_failed:${reason}`);
};
...
for (const name of bundleFiles.slice(0, 3)) {
  assert(sha256(path.join(staging, name)) === attestation.bundleSha256?.[name], `staging_hash:${name}`);
}

const previousTarget = existsSync(current) && lstatSync(current).isSymbolicLink()
  ? path.resolve(canonicalRoot, readlinkSync(current))
  : null;
...
if (previousTarget !== versionDirectory) {
  const temporary = path.join(canonicalRoot, `.current-r64-${randomUUID()}`);
...
const receipt = {
  version: "apr-allocation-react-submit-bundle-install-receipt-r64-v1",
  installedAt: new Date().toISOString(),
  status: "bundle_promoted_local_gate_verified",
  governanceAttestationSha256: sha256(attestationPath),
  versionId,
  previousTarget,
  ...
  operationalReplayPerformed: false,
};
writeFileSync(path.join(ops, "bundle-install-receipt-r64.json"), `${JSON.stringify(receipt, null, 2)}\n`, "utf8");
