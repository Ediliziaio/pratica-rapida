...
const runRoot = path.join(runtimeRoot, "runs", "apr-natural-autonomy11-simple-r53");
const manifestPath = path.join(sourceRoot, "ops/apr-natural-autonomy12-r52-2026-09-04/manifest-simple-remaining11.json");
const sequencerPath = path.join(sourceRoot, "ops/apr-global-controller-test10-2026-08-29/sequencer.mjs");
const bundlePath = path.join(runtimeRoot, "canonical-bundle/versions/67afeee3-fresh-generation-seed-r51-20260904");
...
function makeSingleManifest(item) {
  return {
    version: "apr-natural-autonomy-single-r53-v1",
...
let state = readJson(checkpointPath) ?? {
  version: "apr-natural-autonomy11-simple-r53-checkpoint-v1",
  status: "running",
...
function persist(event, detail = {}) {
  ...
  atomicWrite(reportPath, {
    version: "apr-natural-autonomy11-simple-r53-report-v1",
    status: state.status,
