...
const ops = path.join(repository, "ops/apr-overnight-review-2026-09-03");
const preinstallPath = path.join(ops, "preinstall-gate-attestation-r32.json");
...
const versionId = "24fcd1bf-overnight-review-r32-20260903";
const versionDirectory = path.join(canonicalRoot, "versions", versionId);
const current = path.join(canonicalRoot, "current");
const expectedPrevious = path.join(canonicalRoot, "versions/5156569e-resolved-non-economic-blocker-r31-20260902");
...
const assert = (condition, reason) => { if (!condition) throw new Error(`apr_r32_install_failed:${reason}`); };
...
assert(preinstall.monotonicGate?.candidate?.tests === 1679 && preinstall.monotonicGate?.rulesProved === 106, "preinstall_gate_incomplete");
...
if (previousTarget !== versionDirectory) {
  const temporary = path.join(canonicalRoot, `.current-r32-${randomUUID()}`);
...
const receipt = {
  version: "apr-overnight-review-r32-bundle-install-receipt-v1",
...
const receiptPath = path.join(ops, "bundle-install-receipt-r32.json");
