...
const ops = path.join(repository, "ops/apr-ragni-allocation-delivery-r63-2026-09-05");
...
const versionId = "322df1b9-allocation-single-intent-r63-20260905";
const versionDirectory = path.join(canonicalRoot, "versions", versionId);
const current = path.join(canonicalRoot, "current");
const expectedPrevious = path.join(canonicalRoot, "versions/1bd5ada3-save-material-delivery-r62-20260905");
const bundleFiles = [
  "apr-supervisor.mjs",
  "apr-enea-worker.mjs",
  "apr-watchdog.mjs",
...
const assert = (condition, reason) => {
  if (!condition) throw new Error(`apr_r63_install_failed:${reason}`);
};
...
if (previousTarget !== versionDirectory) {
  const temporary = path.join(canonicalRoot, `.current-r63-${randomUUID()}`);
...
const receipt = {
  version: "apr-allocation-single-intent-bundle-install-receipt-r63-v1",
...
writeFileSync(path.join(ops, "bundle-install-receipt-r63.json"), `${JSON.stringify(receipt, null, 2)}\n`, "utf8");
