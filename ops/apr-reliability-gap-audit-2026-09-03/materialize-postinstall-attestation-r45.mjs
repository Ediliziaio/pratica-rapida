...
const expectedVersionId = "3ecfd0a7-canonical-absence-resume-r45-20260904";
...
const assert = (condition, reason) => { if (!condition) throw new Error(`apr_r45_postinstall_failed:${reason}`); };
const receipt = readJson(path.join(ops, "bundle-install-receipt-r45.json"));
...
const artifact = {
  version: "apr-sequencer-screening-canonical-absence-resume-r45-postinstall-attestation-v1",
  ...
  activeBundleSha256,
  activationBoundary: { canonicalPointerSelected: true, existingKeepaliveProcessWasNotRestarted: true, residentKeepaliveCodePredatesR45: true, nextFreshAprProcessWillLoadR45: true, operationalR45ReplayPerformed: false },
...
const target = path.join(ops, "postinstall-attestation-r45.json");
