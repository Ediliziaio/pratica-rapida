...
if (!existsSync(currentPointer) || !lstatSync(currentPointer).isSymbolicLink()) throw new Error("current_pointer_missing");
const installedVersionId = path.basename(path.resolve(canonicalRoot, readlinkSync(currentPointer)));
const installReceiptPath = path.resolve(import.meta.dirname, "apr-install-document-type-fattura-narrative-batch-r99-2026-09-09-2026-09-08/install-receipt.json");
const installReceipt = JSON.parse(readFileSync(installReceiptPath, "utf8"));
if (installReceipt.versionId !== installedVersionId) throw new Error(`version_mismatch:receipt=${installReceipt.versionId}:disk=${installedVersionId}`);
for (const name of Object.keys(installReceipt.installedBundle)) {
  const activePath = path.join(currentPointer, name);
  if (sha256File(activePath) !== installReceipt.installedBundle[name]) throw new Error(`active_hash_mismatch:${name}`);
}

const cohortRoot = "/Users/giulianolavoro/Library/Application Support/PraticaRapida/enea-shadow-runner/cohorts";
const VALIDATION_REVISION = "apr-r99-document-type-fix-2026-09-09-v2";
...
function reconstruct(dirName, customerKey) {
  ...
  if (!existsSync(commonPath)) return { customerKey, dirName, reconstructed: false, reason: "checkpoint_missing" };
  ...
  return {
    customerKey, dirName, reconstructed: true,
...
const targets = [
  ["apr-pilot-4446-global-controller-natale-tiraboschi", "natale-tiraboschi"],
  ["apr-pilot-2998-global-controller-flavia-cipriani", "flavia-cipriani"],
  ["apr-pilot-4412-global-controller-marco-de-marinis", "marco-de-marinis"],
  ["apr-pilot-4433-global-controller-santo-giuga", "santo-giuga"],
  ["apr-pilot-2999-global-controller-giovanni-amadu", "giovanni-amadu"],
  ["apr-pilot-3019-global-controller-antonino-formisabo", "antonino-formisabo"],
];
const results = targets.map(([dir, key]) => reconstruct(dir, key));
console.log(JSON.stringify({ installedVersionId, results }, null, 2));
