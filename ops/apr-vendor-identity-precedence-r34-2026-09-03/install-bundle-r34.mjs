...
const ops = path.join(repository, "ops/apr-vendor-identity-precedence-r34-2026-09-03");
const preinstallPath = path.join(ops, "preinstall-gate-attestation-r34.json");
...
const versionId = "a5883f66-documentary-demographics-r34-20260903";
const versionDirectory = path.join(canonicalRoot, "versions", versionId);
const current = path.join(canonicalRoot, "current");
const expectedPrevious = path.join(canonicalRoot, "versions/17ed7b59-vendor-identity-precedence-r33-20260903");
...
const assert = (condition, reason) => { if (!condition) throw new Error(`apr_r34_install_failed:${reason}`); };
...
assert(preinstall.monotonicGate?.candidate?.tests === 1693 && preinstall.monotonicGate?.rulesProved === 108, "preinstall_gate_incomplete");
...
if (previousTarget !== versionDirectory) {
  const temporary = path.join(canonicalRoot, `.current-r34-${randomUUID()}`);
...
const receipt = {
  version: "apr-vendor-identity-r34-bundle-install-receipt-v1", installedAt: new Date().toISOString(), status: "bundle_promoted_local_gate_verified",
...
const receiptPath = path.join(ops, "bundle-install-receipt-r34.json");
