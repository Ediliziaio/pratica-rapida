...
const expectedVersionId = "ad840540-recovery-filling-lifecycle-r46-20260904";
...
const assert = (condition, reason) => { if (!condition) throw new Error(`apr_r46_postinstall_failed:${reason}`); };
const receipt = readJson(path.join(ops, "bundle-install-receipt-r46.json"));
...
const artifact = {
  version: "apr-sequencer-screening-recovery-filling-lifecycle-r46-postinstall-attestation-v1",
  ...
  activeBundleSha256,
  activationBoundary: { canonicalPointerSelected: true, existingKeepaliveProcessWasNotRestarted: true, residentKeepaliveCodePredatesR46: true, nextFreshAprProcessWillLoadR46: true, operationalR46ReplayPerformed: false },
...
const target = path.join(ops, "postinstall-attestation-r46.json");
