...
const preinstallPath = path.join(ops, "preinstall-gate-attestation-r42.json");
...
const versionId = "cc879457-probe-lifecycle-r42-20260904";
const versionDirectory = path.join(canonicalRoot, "versions", versionId);
const current = path.join(canonicalRoot, "current");
const expectedPrevious = path.join(canonicalRoot, "versions/1c193d9b-recovery-continuation-r41-20260904");
...
const assert = (condition, reason) => { if (!condition) throw new Error(`apr_r42_install_failed:${reason}`); };
...
assert(preinstall.monotonicGate?.candidate?.tests === 1723 && preinstall.monotonicGate?.rulesProved === 117, "preinstall_gate_incomplete");
assert(preinstall.correction?.status === "proved_positive_negative_restart_and_finalizer", "correction_gate_incomplete");
...
if (previousTarget !== versionDirectory) {
  const temporary = path.join(canonicalRoot, `.current-r42-${randomUUID()}`);
