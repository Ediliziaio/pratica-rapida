...
const gateRoot = path.join(repositoryRoot, "ops/apr-final-total-only-r111-2026-09-11");
...
const historicalAudit = path.join(gateRoot, "historical-audit-report-r111.json");
...
const requiredRuntimeRuleIds = [
  ...
  USER_AUTHORIZED_RULE_IDS.screeningMeasurementsInLineDescription,
  ...
  USER_AUTHORIZED_RULE_IDS.resellerFormCompletionDateOverInvoice,
] as const;
...
if (!report.success || report.numFailedTests !== 0 || report.numPassedTests !== report.numTotalTests) {
  throw new Error("r111_source_test_report_not_green");
}
...
if (missingRuntimeRules.length) throw new Error(`r111_runtime_rules_missing:${missingRuntimeRules.join(",")}`);

const evidence = materializeAprRuleProofs({
  repositoryRoot,
  rootDirectory: evidenceRoot,
  rawReportPath: rawReport,
  testCommand: `vitest completo seriale: ${report.numPassedTests}/${report.numTotalTests} verde`,
});
if (evidence.testedCount !== APR_RULE_TEST_MATRIX.length) {
  throw new Error(`r111_rule_evidence_incomplete:${evidence.testedCount}/${APR_RULE_TEST_MATRIX.length}`);
}
...
if (audit.status !== "incomplete"
  ...
  throw new Error(`r111_historical_gap_not_exactly_authorized:${JSON.stringify(unresolved)}`);
}
