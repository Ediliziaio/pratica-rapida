...
import { replayProductCorpusFiles } from "./aprProductCorpusReplay";

function argument(name: string) {
  const index = process.argv.indexOf(name);
  if (index < 0 || !process.argv[index + 1]) throw new Error(`apr_product_cli_argument_missing:${name}`);
...
async function main() {
  ...
  const first = await replayProductCorpusFiles(replayPath, manifestPath);
  const second = await replayProductCorpusFiles(replayPath, manifestPath);
  const output = {
    ...first,
