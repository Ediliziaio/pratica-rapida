...
const ops = path.join(repository, "ops/apr-targeted-excel-ready-r70-2026-09-06");
const staging = path.join(ops, "staged-bundle-r70");
const canonicalRoot = "/Users/giulianolavoro/Library/Application Support/PraticaRapida/enea-shadow-runner/canonical-bundle";
const versionId = "3a865dd6-gestionale-formal-governance-r70-20260906";
const versionDirectory = path.join(canonicalRoot, "versions", versionId);
const current = path.join(canonicalRoot, "current");
const expectedPrevious = path.join(canonicalRoot, "versions/90435679-comune-rinaldi-alt-verified-r69-20260906");
const executables = ["apr-supervisor.mjs", "apr-enea-worker.mjs", "apr-watchdog.mjs"];
const bundleFiles = [...executables, "apr-rule-governance-attestation.json"];
const sha256 = (target) => createHash("sha256").update(readFileSync(target)).digest("hex");
const assert = (condition, reason) => { if (!condition) throw new Error(`apr_r70_install_failed:${reason}`); };
...
for (const name of executables) assert(sha256(path.join(staging, name)) === attestation.bundleSha256?.[name], `staging_hash:${name}`);
...
for (const name of bundleFiles) {
  ...
  chmodSync(target, name.endsWith(".json") ? 0o600 : 0o700);
  assert(sha256(target) === sha256(source), `installed_hash:${name}`);
}
if (previousTarget !== versionDirectory) {
  const temporary = path.join(canonicalRoot, `.current-r70-${randomUUID()}`);
...
const receipt = {
  version: "apr-targeted-excel-ready-bundle-install-receipt-r70-v1",
  installedAt: new Date().toISOString(), status: "bundle_promoted_formal_governance_verified",
  ...
  correctionsIncluded: [
    "fase CRM gestionale ammessa dal seed APR con identita pratica e stage verificati fail-closed",
    "catalogo prove formali riallineato ai nomi correnti delle fixture",
    "matrice della spesa professionale riallineata alla decisione superseding del 2026-09-06",
  ],
  keepaliveRestartRequested: false, chromeRestartRequested: false, operationalReplayPerformed: false,
};
writeFileSync(path.join(ops, "bundle-install-receipt-r70.json"), `${JSON.stringify(receipt, null, 2)}\n`, "utf8");
