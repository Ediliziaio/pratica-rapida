...
export function buildEneaPortalWorkflowRuntimeScript({
  ...
  return `(async()=>{const steps=${serializedSteps};const screenings=${serializedScreenings};const storageKey=${storageKey};if(!/(^|\\.)enea\\.it$/i.test(location.hostname))throw new Error("Aprire una pagina della pratica sul portale ENEA.");const wait=ms=>new Promise(resolve=>setTimeout(resolve,ms));const normalize=value=>String(value??"").normalize("NFD").replace(/[\\u0300-\\u036f]/g,"").trim().replace(/\\s+/g," ").toLocaleLowerCase("it");const setValue=(element,value)=>{const prototype=element instanceof HTMLSelectElement?HTMLSelectElement.prototype:element instanceof HTMLTextAreaElement?HTMLTextAreaElement.prototype:HTMLInputElement.prototype;const setter=Object.getOwnPropertyDescriptor(prototype,"value")?.set;if(setter)setter.call(element,value);else element.value=value;element.dispatchEvent(new Event("input",{bubbles:true}));element.dispatchEvent(new Event("change",{bubbles:true}));};const chooseAutocomplete=async(element,value)=>{element.focus();setValue(element,value);element.dispatchEvent(new KeyboardEvent("keyup",{key:value.slice(-1),bubbles:true}));const wanted=normalize(value);for(let attempt=0;attempt<25;attempt+=1){await wait(100);const candidates=[...document.querySelectorAll('[role="option"],.ui-autocomplete li,.awesomplete li,.autocomplete-item,.dropdown-menu .dropdown-item,.list-group-item')];const candidate=candidates.find(item=>{const text=normalize(item.textContent);return text===wanted||text.startsWith(wanted+" (")||text.startsWith(wanted+" -")});if(!candidate)continue;const target=candidate.querySelector("a,button")||candidate;for(const type of ["mousedown","mouseup","click"]){target.dispatchEvent(new MouseEvent(type,{bubbles:true,cancelable:true,view:window}));}await wait(150);return normalize(element.value)===wanted||normalize(element.value).startsWith(wanted+" (");}return false;};const screeningOpen=["id-tipo","id-sup_s","id-gtot"].every(id=>document.getElementById(id));let screeningIndex=0;let step;if(screeningOpen&&screenings.length){const stored=Number(sessionStorage.getItem(storageKey)||"0");screeningIndex=Number.isInteger(stored)&&stored>=0&&stored<screenings.length?stored:0;step=screenings[screeningIndex];}else{step=steps.find(candidate=>candidate.markerIds.every(id=>document.getElementById(id)));}if(!step)throw new Error("Questa pagina ENEA non e ancora mappata nel laboratorio.");const result={step:step.id,pageName:step.pageName,compiled:[],notFound:[],notAvailable:[],notSelected:[]};for(const item of step.fields){const element=document.getElementById(item.portalId);if(!element){result.notFound.push(item.portalId);continue;}if(item.control==="autocomplete"){if(!await chooseAutocomplete(element,item.value)){result.notSelected.push(item.portalId);continue;}}else if(item.control==="select"){const wanted=normalize(item.value);const option=[...element.options].find(candidate=>(item.selectValue&&candidate.value===item.selectValue)||normalize(candidate.value)===wanted||normalize(candidate.text)===wanted);if(!option){result.notAvailable.push(item.portalId);continue;}setValue(element,option.value);await wait(50);}else if(item.control==="button"){if(!(element instanceof HTMLElement)||element.hasAttribute("disabled")||element.classList.contains("disabled")){result.notAvailable.push(item.portalId);continue;}for(const type of ["mousedown","mouseup","click"]){element.dispatchEvent(new MouseEvent(type,{bubbles:true,cancelable:true,view:window}));}await wait(150);}else{setValue(element,item.value);}result.compiled.push(item.portalId);}if(screeningOpen&&screenings.length){const form=document.getElementById("id-tipo")?.closest("form");const save=[...(form?.querySelectorAll('button,input[type="submit"]')||[])].find(element=>normalize(element.textContent||element.value)==="salva");if(save&&!save.dataset.eneaLabProgress){save.dataset.eneaLabProgress="1";save.addEventListener("click",()=>sessionStorage.setItem(storageKey,String(Math.min(screeningIndex+1,screenings.length-1))),{once:true});}}console.info("ENEA Lab: pagina riconosciuta e compilata. Nessun salvataggio o invio eseguito.",result);return result;})()`;
}
