...
// Replay locale wide100 POST-ricalcolo completo (r83): a differenza di r71/r73
...
// scritto nei checkpoint dal ricalcolo forzato (full-recalc-r83.mjs): e' il
...
// --- 1. Confermare che il bundle installato e' davvero quello costruito da questo sorgente (r83) ---
if (!existsSync(currentPointer) || !lstatSync(currentPointer).isSymbolicLink()) throw new Error("apr_r83_replay_current_pointer_missing");
...
if (installReceipt.versionId !== installedVersionId) throw new Error(`apr_r83_replay_version_mismatch:receipt=${installReceipt.versionId}:disk=${installedVersionId}`);
for (const name of Object.keys(installReceipt.installedBundle)) {
  const activePath = path.join(currentPointer, name);
  if (sha256File(activePath) !== installReceipt.installedBundle[name]) throw new Error(`apr_r83_replay_active_hash_mismatch:${name}`);
}
...
const payload = {
  version: "apr-wide100-local-replay-r83-v1",
...
if (!verifyImmutableArtifactEnvelope(envelope)) throw new Error("apr_r83_replay_envelope_self_check_failed");
...
const reportMd = `# Replay locale wide100 — r83 (post-ricalcolo completo, stato persistito, hash-verificato)
...
- Fonte: stato persistito nei checkpoint dopo il ricalcolo forzato completo (full-recalc-r83.mjs), non un bypass che ricalcola in isolamento.
