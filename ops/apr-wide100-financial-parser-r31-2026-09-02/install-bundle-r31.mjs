...
const preinstallPath = path.join(ops, "financial-parser-preinstall-gate-attestation-r31.json");
...
const versionId = "5156569e-resolved-non-economic-blocker-r31-20260902";
const versionDirectory = path.join(canonicalRoot, "versions", versionId);
const current = path.join(canonicalRoot, "current");
const expectedPrevious = path.join(canonicalRoot, "versions/68e367e2-rotated-fiscal-bank-layouts-r30-20260902");
...
const assert = (condition, reason) => { if (!condition) throw new Error(`apr_r31_install_failed:${reason}`); };
...
assert(preinstall.monotonicGate?.candidate?.tests === 1662 && preinstall.monotonicGate?.rulesProved === 98, "preinstall_gate_incomplete");
...
if (previousTarget !== versionDirectory) {
  const temporary = path.join(canonicalRoot, `.current-r31-${randomUUID()}`);
...
const receipt = {
  version: "apr-financial-parser-r31-bundle-install-receipt-v1",
  installedAt: new Date().toISOString(), status: "bundle_promoted_pending_runtime_gate",
...
const receiptPath = path.join(ops, "financial-parser-bundle-install-receipt-r31.json");
