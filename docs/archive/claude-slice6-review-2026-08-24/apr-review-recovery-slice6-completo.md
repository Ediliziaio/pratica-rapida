Agisci come revisore tecnico rigoroso dell'architettura monotona APR. Non hai accesso al repository né a Internet: tutto ciò che serve per questa revisione è incluso integralmente in questo unico testo. Non chiedere altri file e non fare affidamento su descrizioni non dimostrate dal codice riportato.

OBIETTIVO DI QUESTA REVISIONE

Revisiona esclusivamente il recovery dello Slice 6 introdotto dal commit:

ccfce1d5d585f47b7b14d991efb28c5ddd91c475 — Riprende l'attivazione APR dopo crash

Il commit originale conteneva una mutazione illegittima del puntatore bundle appartenente allo Slice 5. Tale difetto è stato successivamente corretto dal commit:

2cef33f30b85fae5b1d43dd8b6ca3f7207163976 — Separa il rollback servizi dalla promozione bundle

Devi quindi distinguere rigorosamente:
- il diff storico originale di ccfce1d;
- le parti di quel diff ormai superate da 2cef33f;
- il comportamento effettivo del codice corrente riportato in fondo.

STATO DEI DIECI COMMIT ORIGINALI DELLO SLICE 6

Invariati nel merito dalla correzione 2cef33f:
- 0e39ab4 — collegamento promozione verificata ai plist;
- 4d1f20b — verifica plist sui bundle promossi;
- c683702 — attivazione atomica simulata;
- 9ebd90d — health gate bloccante;
- 49f42b8 — receipt di attivazione;
- aa77f5d — receipt dopo recovery;
- 2383516 — binding e durabilità;
- 5a0fdff — finestra temporale health gate.

Collegati alla mutazione rimossa:
- a114ab5 — rollback normale; già corretto e revisionato;
- ccfce1d — recovery; resta rilevante, ma le parti bundlePointer/previousBundleTarget sono state rimosse da 2cef33f.

L'ordine di revisione resta valido. Questa revisione riguarda soltanto ccfce1d nel suo stato effettivo dopo 2cef33f.

VINCOLI ARCHITETTURALI

- Slice 6 non deve mai mutare il puntatore bundle di Slice 5.
- Un'attivazione fallita deve essere rappresentata dalla receipt di attivazione FAIL.
- Il rollback di Slice 6 può ripristinare soltanto plist e processi simulati.
- Recovery e rollback devono essere idempotenti.
- Nessun CRM, ENEA, browser, SPID, LaunchAgent reale o regola business.
- Nessuna modifica a /api/case-truth o dashboard.
- Non proporre refactoring estetici o teoria generica.

VERIFICHE GIÀ ESEGUITE DOPO 2cef33f

- typecheck runner: PASS;
- suite Slice 1-6: 62/62 PASS;
- suite Infissi: 90/90 PASS;
- suite correlate/dashboard: 103/103 PASS;
- build: PASS;
- test mirati post-commit: 21/21 PASS;
- regressione: dopo health gate fallito il puntatore bundle resta invariato e promoteAprBundles() sullo stesso certificato continua sul percorso idempotente senza hash mismatch.

COSA DEVI CONTROLLARE

1. Recovery dopo crash nella fase POINTER_SWITCHED:
   - può attivare due volte i processi?
   - può dichiarare PASS senza receipt coerente?
   - rispetta la finestra temporale salvata?
   - verifica realmente lo stato dal disco?

2. Recovery durante ROLLING_BACK:
   - ripristina soltanto plist e processi?
   - può ripetere rollback già eseguito?
   - può lasciare lo stato intermedio?
   - verifica correttamente il puntatore plist precedente?

3. Idempotenza:
   - una seconda recovery ripete azioni?
   - activatePreparedAprServices() con activationId già COMPLETE restituisce una receipt coerente?
   - esistono collisioni fra activationId, promotionReceiptId e versionId?

4. Durabilità:
   - transazione, rename, symlink e receipt coprono tutte le finestre di crash?
   - esiste una finestra dopo receipt ma prima di COMPLETE?
   - esiste una finestra dopo rollback processi ma prima della receipt FAIL?

5. Separazione Slice 5/Slice 6:
   - conferma dal codice corrente che nessun percorso del recovery modifica il puntatore bundle;
   - segnala qualsiasi dipendenza residua da previousTarget o promotionRoot che possa reintrodurre il problema.
...
B. PARTI SUPERATE DEL DIFF STORICO
Indica precisamente le righe/logiche di ccfce1d rese obsolete da 2cef33f.

C. DIFETTI CONCRETI NEL CODICE CORRENTE
Per ciascuno:
- gravità;
- file e funzione;
...
- conseguenza;
- correzione minima;
- test automatico richiesto.

D. COPERTURA DEI TEST
Indica soltanto casi realmente mancanti.

E. PROSSIMO PASSO
Se esiste un difetto bloccante, prepara un unico prompt operativo completo per Codex. Se il recovery è approvato, scrivi esplicitamente che si può passare alla revisione di aa77f5d.

Non revisionare ancora gli altri commit. Non inventare problemi non dimostrabili dal codice incluso.

# INIZIO GIT SHOW STORICO ccfce1d

commit ccfce1d5d585f47b7b14d991efb28c5ddd91c475
Author: Giuliano Beretta <giulianolavoro@MBPdiGiiano2026.homenet.telecomitalia.it>
Date:   Mon Aug 24 00:47:08 2026 +0200
