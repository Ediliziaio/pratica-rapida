...
const expectedVersionId = "cc879457-probe-lifecycle-r42-20260904";
...
const assert = (condition, reason) => { if (!condition) throw new Error(`apr_r42_postinstall_failed:${reason}`); };
const receipt = readJson(path.join(ops, "bundle-install-receipt-r42.json"));
...
const artifact = {
  version: "apr-uncertain-save-probe-lifecycle-r42-postinstall-attestation-v1",
  ...
  activeBundleSha256,
  activationBoundary: { canonicalPointerSelected: true, existingKeepaliveProcessWasNotRestarted: true, residentKeepaliveCodePredatesR42: true, nextFreshAprProcessWillLoadR42: true, operationalR42ReplayPerformed: false },
...
const target = path.join(ops, "postinstall-attestation-r42.json");
