...
const expectedVersionId = "8c9c921d-single-product-energy-r37-20260903";
...
const assert = (condition, reason) => { if (!condition) throw new Error(`apr_r37_postinstall_failed:${reason}`); };

const receipt = readJson(path.join(ops, "bundle-install-receipt-r37.json"));
...
const artifact = {
  version: "apr-single-product-energy-r37-postinstall-attestation-v1",
  ...
  activeBundleSha256,
  activationBoundary: { canonicalPointerSelected: true, existingKeepaliveProcessWasNotRestarted: true, residentKeepaliveCodePredatesR37: true, nextFreshAprProcessWillLoadR37: true, operationalR37ReplayPerformed: false },
...
const target = path.join(ops, "postinstall-attestation-r37.json");
