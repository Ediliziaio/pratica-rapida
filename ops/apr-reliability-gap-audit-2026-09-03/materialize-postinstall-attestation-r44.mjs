...
const expectedVersionId = "11757a99-fresh-proof-before-terminal-r44-20260904";
...
const assert = (condition, reason) => { if (!condition) throw new Error(`apr_r44_postinstall_failed:${reason}`); };
const receipt = readJson(path.join(ops, "bundle-install-receipt-r44.json"));
...
const artifact = {
  version: "apr-screening-fresh-proof-before-terminal-r44-postinstall-attestation-v1",
  ...
  activeBundleSha256,
  activationBoundary: { canonicalPointerSelected: true, existingKeepaliveProcessWasNotRestarted: true, residentKeepaliveCodePredatesR44: true, nextFreshAprProcessWillLoadR44: true, operationalR44ReplayPerformed: false },
...
const target = path.join(ops, "postinstall-attestation-r44.json");
