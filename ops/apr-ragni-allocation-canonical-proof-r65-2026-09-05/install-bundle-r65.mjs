...
const ops = path.join(repository, "ops/apr-ragni-allocation-canonical-proof-r65-2026-09-05");
...
const versionId = "27d27bc3-allocation-canonical-proof-r65-20260905";
const versionDirectory = path.join(canonicalRoot, "versions", versionId);
const current = path.join(canonicalRoot, "current");
const expectedPrevious = path.join(canonicalRoot, "versions/3c3f94d6-allocation-react-submit-r64-20260905");
...
const assert = (condition, reason) => { if (!condition) throw new Error(`apr_r65_install_failed:${reason}`); };
...
if (previousTarget !== versionDirectory) {
  const temporary = path.join(canonicalRoot, `.current-r65-${randomUUID()}`);
...
const receipt = {
  version: "apr-allocation-canonical-proof-bundle-install-receipt-r65-v1",
...
writeFileSync(path.join(ops, "bundle-install-receipt-r65.json"), `${JSON.stringify(receipt, null, 2)}\n`, "utf8");
