...
export interface EneaSessionLeaseEvent { id:string; at:string; type:"session_reused"|"keepalive_ok"|"keepalive_failed"|"server_session_verified"; surface:InnocuousEneaSurface; detail:string }
export interface EneaSessionLease { version:typeof ENEA_SESSION_LEASE_VERSION; sessionId:string; status:"active"|"stopped"; startedAt:string; lastKeepaliveAt:string|null; lastServerProofAt:string|null; stopReason:string|null; events:EneaSessionLeaseEvent[] }
...
export function startEneaSessionLease(discovery:EneaSessionDiscovery,now=new Date()):EneaSessionLease|null {
  ...
  return {version:ENEA_SESSION_LEASE_VERSION,sessionId:discovery.sessionId!,status:"active",startedAt:now.toISOString(),lastKeepaliveAt:null,lastServerProofAt:now.toISOString(),stopReason:null,events:[{id:`lease-${now.getTime()}-0`,at:now.toISOString(),type:"session_reused",surface:"dashboard",detail:"Sessione ENEA autenticata esistente riusata; nessuna nuova finestra."}]};
}
export function isAllowedKeepalive(input:{method:string;surface:InnocuousEneaSurface;action:string}):boolean {
  return ["GET","HEAD"].includes(input.method.toUpperCase()) && ["dashboard","summary","allowed_navigation"].includes(input.surface)
    && !/(submit|invia|salva|ricevuta|modify|update|delete)/i.test(input.action);
}
export function recordEneaKeepalive(lease:EneaSessionLease,input:{ok:boolean;serverVerified:boolean;method:string;surface:InnocuousEneaSurface;action:string;reason?:string},now=new Date()):EneaSessionLease {
  if(lease.status!=="active") return lease;
  if(!isAllowedKeepalive(input)) return {...lease,status:"stopped",stopReason:"forbidden_keepalive_action",events:[...lease.events,{id:`lease-${now.getTime()}-${lease.events.length}`,at:now.toISOString(),type:"keepalive_failed",surface:input.surface,detail:"Azione keeper non consentita; parte ENEA sospesa."}]};
  ...
  if(!input.ok||!input.serverVerified) return {...lease,status:"stopped",stopReason:input.reason||"server_session_not_verified",events:[...lease.events,event]};
...
export function stopOverdueEneaLease(lease:EneaSessionLease|null,now=new Date()):EneaSessionLease|null {
  ...
  return {...lease,status:"stopped",stopReason:"keepalive_overdue",events:[...lease.events,{id:`lease-${now.getTime()}-${lease.events.length}`,at:now.toISOString(),type:"keepalive_failed",surface:"dashboard",detail:"Keepalive non verificato entro la finestra consentita; parte ENEA sospesa globalmente."}]};
}
...
export function loadEneaSessionLease(storage:Storage):EneaSessionLease|null {try{const raw=storage.getItem(ENEA_SESSION_LEASE_STORAGE_KEY);if(!raw)return null;const l=JSON.parse(raw) as EneaSessionLease;return l.version===ENEA_SESSION_LEASE_VERSION&&Array.isArray(l.events)?l:null;}catch{return null;}}
