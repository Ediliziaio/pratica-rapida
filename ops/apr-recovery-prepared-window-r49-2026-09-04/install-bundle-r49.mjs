...
const ops = path.join(repository, "ops/apr-recovery-prepared-window-r49-2026-09-04");
const preinstallPath = path.join(ops, "preinstall-gate-attestation-r49.json");
const staging = path.join(ops, "bundle-staging-r49");
const canonicalRoot = "/Users/giulianolavoro/Library/Application Support/PraticaRapida/enea-shadow-runner/canonical-bundle";
const versionId = "9a77fc9f-recovery-prepared-window-r49-20260904";
const versionDirectory = path.join(canonicalRoot, "versions", versionId);
const current = path.join(canonicalRoot, "current");
const expectedPrevious = path.join(canonicalRoot, "versions/ad840540-recovery-filling-lifecycle-r46-20260904");
...
const assert = (condition, reason) => { if (!condition) throw new Error(`apr_r49_install_failed:${reason}`); };
...
assert(preinstall.monotonicGate?.testSuites === 444 && preinstall.monotonicGate?.tests === 1750 && preinstall.monotonicGate?.failedTests === 0 && preinstall.monotonicGate?.rulesProved === 131, "preinstall_gate_incomplete");
assert(preinstall.correction?.ruleId === "system-sequencer-recovery-prepared-window-lifecycle-v1", "correction_gate_incomplete");
...
if (previousTarget !== versionDirectory) {
  const temporary = path.join(canonicalRoot, `.current-r49-${randomUUID()}`);
...
const receipt = {
  version: "apr-recovery-prepared-window-r49-bundle-install-receipt-v1",
...
writeFileSync(path.join(ops, "bundle-install-receipt-r49.json"), `${JSON.stringify(receipt, null, 2)}\n`, "utf8");
