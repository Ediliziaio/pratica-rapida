...
const expectedVersionId = "6481c58d-technical-source-binding-r39-20260903";
...
const assert = (condition, reason) => { if (!condition) throw new Error(`apr_r39_postinstall_failed:${reason}`); };

const receipt = readJson(path.join(ops, "bundle-install-receipt-r39.json"));
...
const artifact = {
  version: "apr-technical-source-binding-r39-postinstall-attestation-v1",
  ...
  activeBundleSha256,
  activationBoundary: { canonicalPointerSelected: true, existingKeepaliveProcessWasNotRestarted: true, residentKeepaliveCodePredatesR39: true, nextFreshAprProcessWillLoadR39: true, operationalR39ReplayPerformed: false },
...
const target = path.join(ops, "postinstall-attestation-r39.json");
