...
const expectedVersionId = "1c193d9b-recovery-continuation-r41-20260904";
...
const assert = (condition, reason) => { if (!condition) throw new Error(`apr_r41_postinstall_failed:${reason}`); };
const receipt = readJson(path.join(ops, "bundle-install-receipt-r41.json"));
...
const artifact = {
  version: "apr-recovery-continuation-and-scoped-binding-r41-postinstall-attestation-v1",
  ...
  activeBundleSha256,
  activationBoundary: { canonicalPointerSelected: true, existingKeepaliveProcessWasNotRestarted: true, residentKeepaliveCodePredatesR41: true, nextFreshAprProcessWillLoadR41: true, operationalR41ReplayPerformed: false },
...
const target = path.join(ops, "postinstall-attestation-r41.json");
