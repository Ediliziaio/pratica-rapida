...
// selettiva (full-reconstruction-r88.mjs, che applica applyValidationRevision
...
const EXPECTED_VALIDATION_REVISION = "apr-full-reconstruction-r88-2026-09-08";

// --- 1. Confermare che il bundle installato e' davvero quello costruito da questo sorgente (r87) ---
if (!existsSync(currentPointer) || !lstatSync(currentPointer).isSymbolicLink()) throw new Error("apr_r88_replay_current_pointer_missing");
...
if (installReceipt.versionId !== installedVersionId) throw new Error(`apr_r88_replay_version_mismatch:receipt=${installReceipt.versionId}:disk=${installedVersionId}`);
for (const name of Object.keys(installReceipt.installedBundle)) {
  const activePath = path.join(currentPointer, name);
  if (sha256File(activePath) !== installReceipt.installedBundle[name]) throw new Error(`apr_r88_replay_active_hash_mismatch:${name}`);
}
...
// cache stantia.
if (notGenuinelyReconstructed.length > 0) throw new Error(`apr_r88_replay_not_genuinely_reconstructed:${JSON.stringify(notGenuinelyReconstructed.map((r) => r.customerKey))}`);
...
const payload = {
  version: "apr-wide100-local-replay-r88-v1",
...
if (!verifyImmutableArtifactEnvelope(envelope)) throw new Error("apr_r88_replay_envelope_self_check_failed");
...
const reportMd = `# Replay locale wide100 — r88 (post-ricostruzione completa mai selettiva, stato persistito, hash-verificato)
...
- Fonte: stato persistito nei checkpoint dopo la ricostruzione forzata e mai selettiva di TUTTE le 100 pratiche (full-reconstruction-r88.mjs); ogni pratica verificata portare la revisione ${EXPECTED_VALIDATION_REVISION} tra quelle applicate, prova che non e' un verdetto vecchio mai ricalcolato.
