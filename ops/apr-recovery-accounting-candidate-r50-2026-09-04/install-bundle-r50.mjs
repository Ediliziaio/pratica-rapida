...
const ops = path.join(repository, "ops/apr-recovery-accounting-candidate-r50-2026-09-04");
const preinstallPath = path.join(ops, "preinstall-gate-attestation-r50.json");
const staging = path.join(ops, "bundle-staging-r50");
const canonicalRoot = "/Users/giulianolavoro/Library/Application Support/PraticaRapida/enea-shadow-runner/canonical-bundle";
const versionId = "c7b93248-recovery-accounting-candidate-r50-20260904";
const versionDirectory = path.join(canonicalRoot, "versions", versionId);
const current = path.join(canonicalRoot, "current");
const expectedPrevious = path.join(canonicalRoot, "versions/9a77fc9f-recovery-prepared-window-r49-20260904");
...
const assert = (condition, reason) => { if (!condition) throw new Error(`apr_r50_install_failed:${reason}`); };
...
assert(preinstall.monotonicGate?.testSuites === 444 && preinstall.monotonicGate?.tests === 1751 && preinstall.monotonicGate?.failedTests === 0 && preinstall.monotonicGate?.rulesProved === 132, "preinstall_gate_incomplete");
assert(preinstall.correction?.ruleId === "system-verified-package-recovery-accounting-candidate-separation-v1", "correction_gate_incomplete");
...
if (previousTarget !== versionDirectory) {
  const temporary = path.join(canonicalRoot, `.current-r50-${randomUUID()}`);
...
const receipt = {
  version: "apr-recovery-accounting-candidate-r50-bundle-install-receipt-v1",
...
writeFileSync(path.join(ops, "bundle-install-receipt-r50.json"), `${JSON.stringify(receipt, null, 2)}\n`, "utf8");
