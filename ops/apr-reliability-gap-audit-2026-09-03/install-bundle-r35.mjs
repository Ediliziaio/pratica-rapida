...
const preinstallPath = path.join(ops, "preinstall-gate-attestation-r35.json");
...
const versionId = "f68568da-fiscal-copy-reliability-r35-20260903";
const versionDirectory = path.join(canonicalRoot, "versions", versionId);
const current = path.join(canonicalRoot, "current");
const expectedPrevious = path.join(canonicalRoot, "versions/a5883f66-documentary-demographics-r34-20260903");
...
const assert = (condition, reason) => { if (!condition) throw new Error(`apr_r35_install_failed:${reason}`); };
...
assert(preinstall.monotonicGate?.candidate?.tests === 1696 && preinstall.monotonicGate?.rulesProved === 110, "preinstall_gate_incomplete");
assert(preinstall.freshOriginalReplay?.improvedCases?.length === 2 && preinstall.freshOriginalReplay?.unchangedCases === 98, "replay_gate_incomplete");
...
if (previousTarget !== versionDirectory) {
  const temporary = path.join(canonicalRoot, `.current-r35-${randomUUID()}`);
...
const receipt = {
  version: "apr-fiscal-copy-reliability-r35-bundle-install-receipt-v1",
...
const receiptPath = path.join(ops, "bundle-install-receipt-r35.json");
