...
const reports = Array.from({ length: 8 }, (_, index) => JSON.parse(readFileSync(
  path.join(directory, `vitest-shard-${index + 1}-r44.json`),
  "utf8",
)));
...
if (!merged.success || merged.numFailedTests || merged.numFailedTestSuites || merged.uniqueTestFileCount !== merged.testFileCount) {
  throw new Error("full_r44_gate_not_green_or_duplicate");
}
writeFileSync(path.join(directory, "full-vitest-r44-final.json"), `${JSON.stringify(merged, null, 2)}\n`);
