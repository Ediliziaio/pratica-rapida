...
const runRoot = path.join(runtimeRoot, "runs", "apr-wide100-final-r66-20260905");
...
const bundlePath = path.join(runtimeRoot, "canonical-bundle/versions/e4afbcb6-final-canonical-proof-r66-20260905");
const expectedManifestSha256 = "51842252f71cabb6714f78923fe59ec9789ad06739393f61a553bb4bdb9154e0";
const expectedWorkerSha256 = "e4afbcb63adbf9b0583f8a5e86cb9a60fb0133bb93740c1275a49f36f516d784";
const cohortOffset = 3170;
...
const manifest = {
  ...sourceManifest,
  version: "apr-wide100-final-r66-execution-envelope-v1",
  experimentId: "apr-wide100-final-r66-20260905",
...
function makeSingleManifest(item) {
  return {
    version: "apr-wide100-final-r66-single-case-v1",
...
let state = readJson(checkpointPath) ?? {
  version: "apr-wide100-final-r66-checkpoint-v1",
  status: "running",
...
function buildReport() {
  const value = counts();
  return {
    version: "apr-wide100-final-r66-report-v1",
    status: state.status,
...
function reportMarkdown(report) {
  const lines = [
    "# APR — collaudo finale delle 100 pratiche, r66",
    "",
...
async function notifyMilestone(processed) {
  ...
  const message = `APR r66 campione 100: ${processed}/100 processate — complete ${report.saved}, intervento operatore ${report.operatorRequired}, blocchi tecnici ${report.technicalBlock}, incoerenti ${report.inconsistent}. Report: ${reportPath}`;
  const notification = {
    processed,
...
atomicWrite(preflightPath, {
  version: "apr-wide100-final-r66-preflight-v1",
