...
const preinstallPath = path.join(ops, "preinstall-gate-attestation-r45.json");
...
const versionId = "3ecfd0a7-canonical-absence-resume-r45-20260904";
const versionDirectory = path.join(canonicalRoot, "versions", versionId);
const current = path.join(canonicalRoot, "current");
const expectedPrevious = path.join(canonicalRoot, "versions/11757a99-fresh-proof-before-terminal-r44-20260904");
...
const assert = (condition, reason) => { if (!condition) throw new Error(`apr_r45_install_failed:${reason}`); };
...
assert(preinstall.monotonicGate?.candidate?.tests === 1728 && preinstall.monotonicGate?.rulesProved === 119, "preinstall_gate_incomplete");
assert(preinstall.correction?.status === "proved_positive_negative_canonical_absence_resume", "correction_gate_incomplete");
...
if (previousTarget !== versionDirectory) {
  const temporary = path.join(canonicalRoot, `.current-r45-${randomUUID()}`);
...
const receipt = {
  version: "apr-sequencer-screening-canonical-absence-resume-r45-bundle-install-receipt-v1",
...
const receiptPath = path.join(ops, "bundle-install-receipt-r45.json");
